  ...
  /*  ++++ uncommented out for task 4*/
	...
	//intset with everything that could start a factor
	static IntSet firstFactor = new IntSet(atomicSym, escapedSym, lbrackSym, lparenSym);
	static IntSet firstAtom = new IntSet (atomicSym, escapedSym);
	...
	static void Term (){
		//Term = Factor { ["."] Factor } .
		Factor();
		while (firstFactor.contains(sym.kind) || sym.kind==dotSym){
			if (sym.kind == dotSym){
	...
	static void Element(){
		...
		else if (firstAtom.contains(sym.kind)){
			Atom();
		}
	}//Element
	static void Range(){
		...
		OneRange();
		while (firstAtom.contains(sym.kind)){ //need a while loop here as there might be many
			OneRange();
		}
	...
	static void Atom (){
		//Atom = atomic | escaped .
		if (sym.kind == escapedSym){
			accept(escapedSym, "escaped character expected");
		}
		else if(sym.kind==atomicSym){
